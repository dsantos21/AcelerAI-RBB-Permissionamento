  ...
  if (!!adminContract) {
    const handleAdd = async (value: string) => {
      try {
        ...
        const votedEvent = receipt.events!.filter(e => e.event && e.event === 'Voted').pop();
        if (!addEvent && !votedEvent) {
          openToast(value, FAIL, `Error while processing Admin account: ${value}`);
        } else if (!addEvent && votedEvent){
          openToast(value, SUCCESS, `Successful vote to add this account as admin: ${value}`);
        } else {
