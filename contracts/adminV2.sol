...
contract Admin is AdminProxy, AdminList {
    ...
    function voteForStructuralChanges (address _address) external onlyAdmin isAdmin(_address) multisigTx(keccak256(abi.encodePacked(_address))) {
        votedForStructuralChanges = _address;
    }

    function addAdmin(address _address) external onlyAdmin multisigTx(keccak256(abi.encodePacked(_address))) {
    ...
    function removeAdmin(address _address) external onlyAdmin atLeastOneAdmin multisigTx(keccak256(abi.encodePacked(_address))) {
    ...
    function changeMajorityRequirement() external onlyAdmin multisigTx(keccak256(abi.encodePacked(qualifiedMajority))) {
