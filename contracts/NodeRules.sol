...
contract NodeRules is NodeRulesProxy, NodeRulesList {
    ...
    modifier multisigTx(bytes32 hash) {
        ...
        if(Admin(adminContractAddress).voteAndVerify(hash, msg.sender)){
            _;
        }
    }
    ...
    function addEnode(
        ...
        uint16 port
    ) external onlyAdmin onlyOnEditMode multisigTx(keccak256(abi.encodePacked(ADD_ENODE, enodeId, host, port))) {
    ...
    function removeEnode(
        ...
        string calldata host,
        uint16 port
    ) external onlyAdmin onlyOnEditMode multisigTx(keccak256(abi.encodePacked(REMOVE_ENODE, enodeId, host, port))) {
