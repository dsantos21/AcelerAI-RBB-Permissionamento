...
contract NodeRules is NodeRulesProxy, NodeRulesList {
    ...
    modifier multisigTx(string memory opName, string memory enodeId, string memory host, uint16 port) {
        address adminContractAddress = nodeIngressContract.getContractAddress(nodeIngressContract.ADMIN_CONTRACT());
        if(Admin(adminContractAddress).voteAndVerify(opName, msg.sender, msg.sender)){ // ta errado
            _;
        }
    }
    ...
    function addEnode(
        ...
        uint16 port
    ) external onlyAdmin onlyOnEditMode multisigTx(ADD_ENODE, enodeId, host, port) {
    ...
    function removeEnode(
        ...
        string calldata host,
        uint16 port
    ) external onlyAdmin onlyOnEditMode multisigTx(REMOVE_ENODE, enodeId, host, port) {
